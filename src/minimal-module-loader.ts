...
export interface ModuleLoader {
  ...
  processScript: (scriptCode: string, moduleKey?: string) => Promise<ModuleExports>;
...
window.createModuleLoader = function(baseUrl: string = ''): ModuleLoader {
  ...
  function transformExports(code: string, moduleKey: string): string {
    ...
    const sortedExports = exportDeclarations.sort((a, b) => (b.end || 0) - (a.end || 0));
    ...
    for (const exportNode of sortedExports) {
      ...
      if (exportNode.type === 'ExportDefaultDeclaration') {
        if (exportNode.declaration.type === 'FunctionDeclaration' && exportNode.declaration.id) {
          ...
          const funcCode = code.substring(exportNode.declaration.start, exportNode.declaration.end);
          transformedCode = transformedCode.substring(0, start) +
                           funcCode +
          ...
          const classCode = code.substring(exportNode.declaration.start, exportNode.declaration.end);
          transformedCode = transformedCode.substring(0, start) +
                           classCode +
                           transformedCode.substring(end);
          ...
          const defaultValue = code.substring(exportNode.declaration.start, exportNode.declaration.end);
          transformedCode = transformedCode.substring(0, start) +
                           `const __default = ${defaultValue};` +
                           transformedCode.substring(end);
          additions += `\nexports["${moduleKey}"] = exports["${moduleKey}"] || {}; exports["${moduleKey}"]["default"] = __default;`;
        }
      } else if (exportNode.type === 'ExportNamedDeclaration' && exportNode.declaration) {
        if (exportNode.declaration.type === 'FunctionDeclaration' && exportNode.declaration.id) {
          const funcName = exportNode.declaration.id.name;
          const funcCode = code.substring(exportNode.declaration.start, exportNode.declaration.end);
          transformedCode = transformedCode.substring(0, start) +
          ...
          const declCode = code.substring(exportNode.declaration.start, exportNode.declaration.end);
          transformedCode = transformedCode.substring(0, start) +
                           declCode +
                           transformedCode.substring(end);
      ...
      } else if (exportNode.type === 'ExportNamedDeclaration' && exportNode.specifiers) {
        // export { a, b }
        const specifiers = exportNode.specifiers || [];
        transformedCode = transformedCode.substring(0, start) + transformedCode.substring(end);
  ...
  function isRelativeImport(source: string): boolean {
    ...
    if (source.startsWith('/node_modules/') || source.startsWith('node_modules/')) return true;
  ...
  function transformImports(code: string, currentModuleKey: string = ''): string {
    ...
    try {
      ...
      function findImports(node: any, depth = 0) {
        ...
        if (node.type === 'Import' || node.type === 'ImportExpression') {
          ...
          if (node.type === 'ImportExpression') {
            ...
            if (source && isRelativeImport(source)) {
              console.log('🔍 DEBUG ImportExpression is relative, adding to imports list');
              ...
              console.log('🔍 DEBUG ImportExpression is NOT relative, skipping:', source);
            }
          }
        }
        ...
        if (node.type === 'ImportDeclaration') {
          ...
          const calleeName = node.callee?.name || node.callee?.type || 'unknown';
          if (depth < 8) {
            console.log(`🔍 DEBUG CallExpression at depth ${depth}, callee:`, calleeName);
          }

          // Check all CallExpressions to see what we're dealing with
          if (node.callee?.type === 'Import') {
            const source = node.arguments?.[0]?.value || '';
            console.log('🔍 DEBUG found dynamic import CallExpression (Import callee):', source);
            importDeclarations.push({
              type: 'DynamicImport',
              start: node.start,
              end: node.end,
            ...
            const source = node.arguments?.[0]?.value || '';
            console.log('🔍 DEBUG found dynamic import CallExpression (import identifier):', source);
            importDeclarations.push({
              type: 'DynamicImport',
              start: node.start,
              end: node.end,
            ...
            console.log('🔍 DEBUG found dynamic import in MemberExpression (.then pattern):', source);
            ...
            // Log all CallExpressions to debug
            if (node.arguments?.[0]?.value && typeof node.arguments[0].value === 'string' &&
                node.arguments[0].value.includes('./assets/')) {
              console.log('🔍 DEBUG found CallExpression with asset path:', calleeName, node.arguments[0].value);
            }
      ...
      console.log('🔍 DEBUG total imports found:', importDeclarations.length);    let transformedCode = code;

    // Process imports from end to start to maintain positions
    const sortedImports = importDeclarations.sort((a, b) => (b.end || 0) - (a.end || 0));
    ...
    for (const importNode of sortedImports) {
      ...
      if (importNode.type === 'DynamicImport') {
        ...
        if (isRelativeImport(source)) {
          ...
          console.log('🔄 DEBUG transforming dynamic import from:', source, 'to loadModule call for:', moduleKey);

          transformedCode = transformedCode.substring(0, start) +
                           replacement +
                           transformedCode.substring(end);
        } else {
          console.log('🔄 DEBUG skipping non-relative dynamic import:', source);
        }
      } else {
  ...
  function transformExportFrom(code: string, moduleKey: string): string {
    ...
    const sortedExports = exportFromDeclarations.sort((a, b) => (b.end || 0) - (a.end || 0));
...
`;
  ...
  function resolveModulePath(importPath: string, currentModuleKey: string): string {
    ...
    if (importPath.startsWith('http://') || importPath.startsWith('https://')) {
      try {
        ...
        if (url.origin === baseUrlObj.origin) {
          ...
          if (baseUrlObj.pathname !== '/' && resolved.startsWith(baseUrlObj.pathname.substring(1))) {
            resolved = resolved.substring(baseUrlObj.pathname.length - 1);
          }
          console.log('🔧 DEBUG resolveModulePath resolved absolute URL to:', resolved);
          return resolved;
        } else {
  ...
  async function processModule(moduleKey: string): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to load ${moduleKey}: ${response.status} ${response.statusText}`);
      }

      // Check if we got HTML instead of JavaScript (common with incorrect URLs)
      const contentType = response.headers.get('content-type') || '';
      if (contentType.includes('text/html')) {
        throw new Error(`Received HTML instead of JavaScript for ${moduleKey}. Check if the URL is correct: ${url}`);
      }
      ...
      console.log('📦 DEBUG processModule transformed code length:', code.length);
  ...
  return {
    async loadScript(url: string, moduleKey?: string): Promise<ModuleExports> {
      ...
      const promise = (async () => {
        ...
        console.log('📦 DEBUG loadScript transformed code length:', code.length);
    ...
    async processScript(scriptCode: string, moduleKey: string = 'main'): Promise<ModuleExports> {
      console.log('📦 DEBUG processScript original code length:', scriptCode.length);
      ...
      console.log('📦 DEBUG processScript transformed code length:', code.length);
    ...
    loadModule,
    getModule: (moduleKey: string): ModuleExports | undefined => exports[moduleKey],
    isLoaded: (moduleKey: string): boolean => !!exports[moduleKey] && !exports[moduleKey]._loading,
    clear: (): void => {
      cache.clear();
      processing.clear();
